...
async fn subscribe_returns_a_400_when_fields_are_present_but_invalid() {
    let app = spawn_app().await;
    let test_cases = vec![
        ("name=&email=monospace%40gmail.com", "empty name"),
        ("name=mono&email=", "empty email"),
        ("name=mono&email=invalid-email", "invalid email"),
    ];
...
#[tokio::test]
async fn subscribe_fails_is_there_is_a_fatal_database_error() {
    // arrange
    let app = spawn_app().await;
    let body = "name=mono&email=monospace%40gmail.com";
